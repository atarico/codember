const politicsKeys = `8-10 r: ozrcdfnug
9-10 q: hvsazxrigf
1-9 j: bbqonxjt
...
const finalMessage = politicsKeys.split('\n').filter((onePoliticKey) => {
    const singleElement = onePoliticKey.split(':')
    const rangeAndLetter = singleElement[0].split(' ')
    const corruptedPassword = singleElement[1].trim()
    const range = rangeAndLetter[0].split('-')
    const numberOfLetterInMessage = corruptedPassword
        .split('')
        .filter((letter) => letter === rangeAndLetter[1]).length


    return !(numberOfLetterInMessage >= range[0] && numberOfLetterInMessage <= range[1])
})
console.log(finalMessage[41])
